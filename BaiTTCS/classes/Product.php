	...
	class product
	{
		...
		public function insert_product($data,$files){
			...
			if($productName=="" || $brand=="" || $category=="" || $product_desc=="" || $price=="" || $type=="" || $file_name ==""){
				...
				if($result){
					$alert = "<span class='success'>Chèn sản phẩm thành công</span>";
					return $alert;
				}else{
					$alert = "<span class='error'>Chèn sản phẩm ko thành công</span>";
					return $alert;
				}
			}
		}
		...
		public function show_product(){
			...
			SELECT product.*, category.catName, brand.brandName 
		...
		public function getLastestKlipsch(){
			$query = "SELECT * FROM product WHERE brandId = '10' order by productId desc LIMIT 1";
			$result = $this->db->select($query);
			return $result;
		}
		...
		public function insert_slider($data,$files){
			...
			if($sliderName=="" || $type==""){
				...
				if(!empty($file_name)){
					...
					$result = $this->db->insert($query);
					if($result){
						$alert = "<span class='success'>Đã thêm thanh trượt thành công</span>";
						return $alert;
					}else{
						$alert = "<span class='error'>Đã thêm thanh trượt không thành công</span>";
						return $alert;
					}
				}
